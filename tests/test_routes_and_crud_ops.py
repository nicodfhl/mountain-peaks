...
from mountain_peaks.backend.app.crud_ops import (
    add_a_peak,
    delete_a_peak,
    get_a_peak_by_id,
...
class TestRoutes:
    ...
    def compare_peaks(self, out_data, in_data, peak_id=None):
        if peak_id:
    ...
    def test_endpoint_2_add_peak(self):
        ...
        self.compare_peaks(out_data=resp_1.json(), in_data=in_data)
        resp_2 = test_client.get("/peaks/")
        assert resp_2.status_code == 200, resp_2.text
        out_data = resp_2.json()
        assert len(out_data) == 1
        self.compare_peaks(out_data=out_data[0], in_data=in_data)
    ...
    def test_endpoint_3_read_peak(self):
        in_data = {
            "name": "Test Peak 2",
            "height": 2587.0,
            "latitude": -60,
        ...
        assert resp.status_code == 200, resp.text
        self.compare_peaks(out_data=resp.json(), in_data=in_data, peak_id=peak_id)
    ...
    def test_endpoint_4_update_peak(self):
        # Create a peak
        in_data = {
            "name": "Test Peak 1",
            "height": 2587.0,
        ...
        assert resp.status_code == 200, resp.text
        self.compare_peaks(out_data=resp.json(), in_data=up_data, peak_id=peak_id)
        up_data_partial = {
            "height": 2222,
            "longitude": 56,
        }
        ...
        self.compare_peaks(out_data=resp_p.json(), in_data=up_data, peak_id=peak_id)


    def test_endpoint_5_delete_peak(self):
        peak_id = 1
        ok_resp = test_client.get(f"/peaks/{peak_id}")
        del_resp = test_client.delete(f"/peaks/{peak_id}")
        assert del_resp.status_code == 200, del_resp.text
        self.compare_peaks(out_data=del_resp.json(), in_data=ok_resp.json(), peak_id=peak_id)
        # Try to get the deleted peak
        ko_resp = test_client.get(f"/peaks/{peak_id}")
        assert ko_resp.status_code == 404, ko_resp.text


    def test_endpoint_7_get_peaks_inside_bbox(self):
        # Create a peak
        in_data_1 = {
            "name": "Test Peak 1",
            "height": 2587.0,
...
@pytest.fixture
def t_session() -> Generator[Session, None, None]:
...
class TestOperations:
    ...
    def test_operation_2_add_peak(self, t_session: Session) -> None:
        # Create a new peak and check if the response is ok
        peak = add_a_peak(session=t_session,
                        peak=PeakCreate(
                            name="Test Peak 4321",
                            height=4321,
                            latitude=-43.21,
    ...
    def test_operation_5_delete_peak(self, t_session: Session):
        # Add a peak
        peak = add_a_peak(session=t_session,
                          peak=PeakCreate(
                            name="Test Peak 4321",
        ...
        peak_id = peak.pid
        # Try to get the deleted peak
        del_peak = delete_a_peak(session=t_session, peak_id=peak.pid)
        ...
        with pytest.raises(Exception):
            get_a_peak_by_id(session=t_session, info=peak_id)

    def test_operation_6_find_peaks_by_attr(self, t_session: Session):
        # method used by get_peaks_from_attr route
        # Add 1 peak (already one in deb lat:0, lon:0)
        peak_inside = add_a_peak(session=t_session,
                        peak=PeakCreate(
                            name="Test Peak attr",
        ...
        f_peak = find_peaks_by_attr(session=t_session, attr=peak_attr)[0]
        ...
        f_peaks = find_peaks_by_attr(session=t_session, attr=peak_attr)
        assert len(f_peaks) == 2
        assert sorted([f_peaks[0].name, f_peaks[1].name]) == ["Default Peak 000", "Test Peak attr"]
        # try to find a none existing peak
        no_peak_attr = PeakAttr(name="NOT EXISTING PEAK")
        with pytest.raises(PeakNotFoundException):
            find_peaks_by_attr(session=t_session, attr=no_peak_attr)

    def test_operation_7_find_peaks_into_bbox(self, t_session: Session):
        ...
        peaks_all_inside = find_peaks_into_bbox(session=t_session, bbox=bbox_all)
        ...
        peaks_0_inside = find_peaks_into_bbox(session=t_session, bbox=bbox_0)
        ...
        peaks_n_inside = find_peaks_into_bbox(session=t_session, bbox=bbox_n)
        ...
        assert len(find_peaks_into_bbox(session=t_session, bbox=bbox_no)) == 0
